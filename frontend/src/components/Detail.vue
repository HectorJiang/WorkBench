          ...
          <div id="respond-post-354" class="respond border-top">
            <div class="cancel-comment-reply">
              <a
                ...
                href="https://www.misterma.com/archives/354/#respond-post-354"
                rel="nofollow"
                style="display: none"
            ...
            <form
              method="post"
              action="https://www.misterma.com/archives/354/comment"
              id="comment-form"
              role="form"
            >
              <div class="row">
              ...
              <input
                type="hidden"
                name="_"
                value="a588f095e96a9cb14fe5cebbdff03109"
              />
            </form>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    //请求数据
    getData() {
      this.$http.get("/api/v1/article/detail/" + this.$route.params.id).then(
        (res) => {
  ...
  //生命周期函数：挂载完成
  updated() {
    VditorPreview.preview(this.$refs.prev, this.article.article_content, {
      hljs: {
        style: "native",
        lineNumber: true,
      },
